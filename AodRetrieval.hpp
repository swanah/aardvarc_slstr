...
const float inst_frac_error[] = {.024, .032, .02, .033}; /* Instrument calibration errors adjusted to uncertainty document specs (taken from ORAC)) */
...
const float m_error_offset_v[] = {0.01, 0.01, 0.04, 0.02}; /* Land surface model error estimates per channel, derived from MOnte Carlo simulations */
const float m_error_offset_b[] = {0.01, 0.01, 0.01, 0.15}; /* Land surface model error estimates per channel, derived from MOnte Carlo simulations */
//const float m_error_gain[] = {0.07, 0.07, 0.15, 0.1}; /* Written as sum of absolute ('offset') and relative ('gain') errors, 1sd */
const float m_error_gain_v[] = {0.05, 0.05, 0.10, 0.07}; /* Written as sum of absolute ('offset') and relative ('gain') errors, 1sd */
const float m_error_gain_b[] = {0.05, 0.05, 0.05, 0.2}; /* Written as sum of absolute ('offset') and relative ('gain') errors, 1sd */
const float rt_abs_error[] = {.006, .006, .006, .006}; /* Approximate uncertainty in surface reflectance from 6S code for known constituents (from Kalashnikova et al) */
const float aer_mod_frac_error[] = {0.05, 0.05, 0.05, 0.05}; /* Fractional error per channel in aerosol scattering coefficient (multiple of SSA and phase fn errors */
...
const float aer_mod_frac_error_d[] = {0.05, 0.05, 0.05, 0.2}; /* Fractional error per channel in aerosol scattering coefficient (multiple of SSA and phase fn errors */
...
public:
    ...
    Doub operator()(const Doub fot) {
        Brent brent;
        brent.ax = pix.ax;
        brent.bx = 1.05 * pix.ax; //(pix.ax + pix.cx) / 2;
        brent.cx = pix.cx;
        emodTau->fot = fot;
        float fmin, penalty;
        ...
        brent.minimize(*emodTau); //brent_d(&ax, bx, &cx, fct_emod_tau, TOL, &pix.aod);
        fmin = brent.fmin;
        pix.aod = brent.xmin;
    ...
    Doub operator() (VecDoub_I &p){
        ...
        for (i = 0; i < N_SLSTR_BANDS; i++) {
            for (j = 0; j < N_SLSTR_VIEWS; j++) {
                ...
                dir = (1.0 - DF * (double) pix.DD[i][j])*(double) p[4 + j]*(double) p[i];
                g = (1.0 - p6)*(double) p[i];
        ...
        lim = 0.49; if (p[4] < lim) tot = tot + (lim - p[4])*(lim - p[4])*1000.0;
        lim = 0.51; if (p[4] > lim) tot = tot + (lim - p[4])*(lim - p[4])*1000.0;
        lim = 0.20; if (p[5] < lim) tot = tot + (lim - p[5])*(lim - p[5])*1000.0;
...
public:
    ...
    Doub operator() (Doub tau){
        ...
        Doub p[] = {0.1, 0.1, 0.1, 0.1, 0.5, 0.3};
...
public:
    ...
    Doub operator() (Doub tau){
        ...
        for (i = 1; i < N_SLSTR_BANDS; i++){
            for (j = 0; j < N_SLSTR_VIEWS; j++){
                if (pix.RR[i][j] < -1e-3){
                    fmin = fmin + (pix.RR[i][j] * pix.RR[i][j]) * 1000.0; //1000000.0;
                }
                y = (pix.tarr[i][j] + dToa - pix.inv_coef[i][j].rPath) / pix.inv_coef[i][j].tGas;
                y /= (pix.inv_coef[i][j].tDown * pix.inv_coef[i][j].tUp);
                dRsurf_dToa[i*N_SLSTR_VIEWS + j] = fabs((y / (1.0 + pix.inv_coef[i][j].spherAlb * y)) - pix.RR[i][j]) / dToa;
